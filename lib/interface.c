...
void ife_print_long(struct interface *ptr)
{
    ...
    printf(_("%s\t%s<%d>  MTU:%d"),
	   ptr->name, flags, ptr->flags, ptr->mtu);
    ...
    printf("        Link encap:%s", hw->title);
    if (hw->print != NULL && (! (hw_null_address(hw, ptr->hwaddr) &&
				  hw->suppress_null_addr)))
	printf(_("  HWaddr %s"), hw->name, hw->print(ptr->hwaddr));
